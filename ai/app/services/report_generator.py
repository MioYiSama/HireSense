...
class ReportBackend(BaseModel):
    ...
    # 专业能力维度
    distributed_systems: float = Field(description="用户的分布式能力评分")
    performance: float = Field(description="用户的性能把控能力评分")
    database: float = Field(description="用户的数据库能力评分")
    api: float = Field(description="用户的 API 设计与实现能力评分")
    security: float = Field(description="用户的安全意识/能力评分")
    devops: float = Field(description="用户的 DevOps 能力评分")
...
class FinalReport:
    ...
    async def build_report(self, state: AgentState) -> ReportContent:
        ...
        for log in state.interview_logs:
            history_transcript += f"问题:{log.interviewer}回答{log.interviewee}评分:{log.final_score}\n--------------\n"
        ...
        if state.job == "backend":
            specific_dimensions_instruction = (
                "specific维度固定为：分布式、性能、数据库、API、安全、DevOps。"
            )
        elif state.job == "frontend":
            specific_dimensions_instruction = (
                "specific维度固定为：设计、性能、工程化、组件化、数据流、安全。"
            )
        else:
            raise ValueError(
                f"Invalid job type: {state.job}. Expected 'backend' or 'frontend'."
            )
        ...
        prompt = await prompt_tmp.ainvoke(
            {
                ...
                "data": data,
                "specific_dimensions_instruction": specific_dimensions_instruction,
            }
        )
