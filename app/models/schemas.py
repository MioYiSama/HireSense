...
from typing import Optional,List,Dict,Optional,Union,Literal
...
class StartRequest(BaseModel):
    ...
    job : Literal["backend","frontend"] = Field(description="可选类 只有backend和frontend")
    personalization : Optional[str] = Field(description="用户倾向风格")
    resume : str = Field(description="简历信息")
...
class AgentState(BaseModel):
    ...
    probe_num : int = 0
    MAX_probe_num : int = 3
    current_concept_cumulative_answer : str # 用于对于多次追问时 答案的拼接 用来最终形成准确答案
