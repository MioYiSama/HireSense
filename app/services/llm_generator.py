...
class TacticalDecision(BaseModel):
    ...
    action: Literal["PROBE", "TRANSITION"] = Field(
        description="PROBE: 决定继续在当前题进行追问；TRANSITION: 决定放弃当前题，切换新考点。"
    )
    selected_node: Optional[str] = Field(
        default=None,
        description="如果 action 为 TRANSITION，必须从提供的【图谱菜单】中挑选下一个考点；如果为 PROBE，填 None。"
    )
    reply_speech: str = Field(
        description="如果 PROBE，输出带有 Hint 的引导话术；如果 TRANSITION，输出对本题的总结并抛出 selected_node 的新问题。"
    )
    q_id_and_brief: Optional[List[str]] = Field(
        default=None,
        description="如果确定更换考点并选择了所要考察的题目，则填入 [题目对应的q_id,题目内容] ，否则填None"
    )
...
class LLMGenerator:
    def __init__(self ,llm = ChatOpenAI, fast_llm =ChatOpenAI):
        os.environ["DASHSCOPE_API_KEY"] = "sk-1eb5bba60a554e0dad85214f7f24cbce"
    ...
    async def decide_tactics_and_generate(self,
            ...
            prompt = ChatPromptTemplate.from_messages([
                ...
                【你的双重任务】：
                结合【对话历史】和候选人的【累计回答】【简历信息】，决定是继续追问还是更换题目。
                ...
                2. action: 必须是 "PROBE" 或 "TRANSITION"（字符串）
                3. selected_node: 如果 action 是 TRANSITION，填写选中的考点名称；如果是 PROBE，填 null（字符串或null）
                4. reply_speech: 你对候选人说的话，必须口语化、自然（字符串）
                5. q_id_and_brief: 如果 action 是 TRANSITION 且选择了题目，填写 [题目id, 题目内容]；否则填 null（数组或null）

                注意：action 和 reply_speech 是必需字段，不能省略！"""),
