...
async def start_interview(
        req: StartRequest,
        workflow=Depends(get_agent_workflow)
):
    """
    ...
    try:
        ...
        # 严格遵守 API 契约，仅返回 success
        return StartResponse(success="true", reply_speech=res["reply_speech"])

    except Exception as e:
        print(f"[Start API Error]: {e}")
        return StartResponse(success="false", reply_speech="服务器连接失败")
...
async def reply_interview(
    ...
    try:
        ...
        reply_speech = await workflow.process_turn(state, user_text)
...
async def mock_main_server_receive_report(req: ReportPushRequest):
    ...
    print(f"💾 [Mock 主服务器] 正在将雷达图数据和 15 道题的问答明细落入 MySQL 数据库...\n")
